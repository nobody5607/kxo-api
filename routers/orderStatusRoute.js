...
orderStatusRoute.get("/", Auth, async (req, res) => {
  try {
    const result = await OrderStatus.find().sort({ name: 1 });
    res.json(result);
  } catch (error) {
    res.json(error);
  }
});
orderStatusRoute.get("/:id", Auth, async (req, res) => {
  try {
    const { id } = req.params;
...
orderStatusRoute.post("/", Auth, async (req, res) => {
  try {
    const packageData = {
      name: "CELINE",
      brand: [
